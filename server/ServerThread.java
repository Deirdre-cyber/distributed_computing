...
class ServerThread implements Runnable {
   ...
   public void run() {
      ...
      try {
         ...
         log.info("Session ID: " + sslSession.getId());
         ...
         if (sslSession.isValid()) {
            ...
            log.warning("SSL session is invalid");
            myDataSocket.close();
         }
      } catch (SSLHandshakeException e) {
         log.warning("SSL Handshake exception: " + e.getMessage());
         try {
            myDataSocket.close();
         } catch (IOException e1) {
            log.severe("Error closing connection: " + e1.getMessage());
         }
   ...
   private void login(String credentials) {
      try {
         ...
         if (parts.length != 2 || parts == null) {
            message.append(
                  "| 103 Invalid credentials format |\n| Please try again               |\n| Check logs for details         |\n");
